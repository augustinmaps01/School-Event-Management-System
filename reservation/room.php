...
<body class="hold-transition skin-blue sidebar-mini">
  ...
  <!-- Content Wrapper. Contains page content -->
        ...
        <li><a href="room.php"><i class = "fa fa-clone"></i>Reserve Room </a></li>
            ...
            }else if(isset($_GET['exist']) && $_GET['exist'] == 'error' ){
                 echo '<div class = "alert alert-danger" style ="font-size:25px; text-align:center;"><strong><i class = "fa fa-warning"></i> This Date is already reserve!!</strong></div> ';
            }
            ?>
                <div class="row">
                    <div class="col-xs-12">
        ...
        <div class="box-body">
<?php
include_once('reservation_modal.php');
...
?>
                                ...
                                <table id="example1" class="table table-bordered">
                                    ...
                                    members.lastname, reservation.start_date, reservation.end_date, reservation.status,
                                     reservation.timestamp, reservation.start_time, reservation.end_time FROM reservation LEFT JOIN room ON reservation.room_id = room.room_ID
                                 ...
                                 "// <a href = '#sms".$row['reservation_id']."' class = 'btn bg-navy btn-sm' data-toggle = 'modal'><span class = 'glyphicon glyphicon-envelope'></span></a> 
                                   ."
                              "//     <a href = '#status".$row['reservation_id']."'class = 'btn btn-info btn-sm'data-toggle = 'modal'><i class = 'fa fa-info'></i></a>
                              ."     
                              </td>
                                      </tr>";
  ...
  $.ajax({
    ...
    success: function(response){
     ...
     $('#edit_end').val(response.end_date);
      $('#s_time').val(response.start);
        $('#e_time').val(response.end);
        $('#stat').val(response.status).html(response.status);
